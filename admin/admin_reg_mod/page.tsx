...
interface ModeratorFormData {
  firstName: string;
  lastName: string;
  email: string; // Add email field to the form data interface
  contact: string;
  password: string;
}
...
export default function AdminRegistration() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/admin/register/moderator', {
        firstName,
        lastName,
        email,
        contact,
      ...
      router.push('http://localhost:3001/admin/admin_reg_mod'); // Redirect to the specified URL

    } catch (error) {
      console.error('Error registering moderator:', error);
      toast.error('Moderator registration failed. Please try again.');
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div>
          <Toaster />
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">Moderator Registration</h2>
        </div>
